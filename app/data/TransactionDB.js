...
const getTransactions = (onGetTransactions) => {
  ...
  db.transaction((tx) => {
    tx.executeSql(
      "select t.id, t.amount, t.description, t.category, t.date, t.account_id, a.name, a.type from transactions t inner join accounts a on t.account_id=a.id",
      [],
      (_, { rows: { _array } }) => {
...
const addTransaction = (transaction, onTransactionAdded) => {
  ...
  db.transaction(
    (tx) => {
      tx.executeSql(
        "insert into transactions (amount, description, date, category, account_id, created_at) values (?, ?, ?, ?, ?, ?)",
        [
          transaction.amount,
